...
function normalizeItem(item, categoryId) {
  return {
    ...
    heroOnly: Boolean(item.heroOnly),
...
function getFallbackImages() {
  return (fallbackPortfolio.images || []).map((group, index) => ({
    id: null,
    ...
    iconS3Key: group.iconS3Key || "",
    order: index,
    subcategories: normalizeSubcategories(group.subcategories),
...
export function getFallbackPortfolio() {
  ...
  const featuredImages = images.flatMap((group) =>
    group.items
      .filter((item) => item.featured)
      .map((item) => ({ ...item, categoryName: group.category, categorySlug: group.slug })),
  )
...
export async function getCategoryBySlug(slug) {
  if (!isMongoConfigured()) {
    const fallbackCategory = getFallbackImages().find((item) => item.slug === slug)
  ...
  const images = await db
    .collection("images")
    .find({ categoryId: category._id, $or: [{ heroOnly: { $exists: false } }, { heroOnly: false }] })
    .sort({ createdAt: -1 })
    .toArray()
  ...
  return serializeCategory(category, imagesByCategoryId)
}
...
export async function getSubcategoryBySlugs(categorySlug, subcategorySlug) {
  ...
  return {
    ...category,
    subcategory,
    items: (category.items || []).filter((image) => image.subcategoryId === subcategory.id),
  }
}
